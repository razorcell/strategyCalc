...
var weights = [0.60, 0.40]; // probabilities
...
let after_consecutive_wins = false;
  ...
  if(outscomes_buffer.length == 3){
    ...
    if(outscomes_buffer[0] == 'WIN' && outscomes_buffer[1] == 'WIN' && outscomes_buffer[2] == 'WIN'){
    ...
    if(target == 0.25 || target == 0.50 || target == 0.75){
      // console.log(`Force Consecutive losses --->`);
      for(i=0;i<consecutive_losses;i++){//filling losses array
        losses_array.push('LOSS');
      }
    }
    if(losses_array.length != 0){//array of losses is full
